...
def _close_tf_singleton_mutex(pid: int) -> bool:
    """
    Закрытие хэндла мутанта для tf.exe, чтобы разрешить множественный запуск.
...
def _close_all_cs2_singleton_mutexes(primary_pid: int | None = None) -> bool:
    ...
    проходит по всем запущенным tf.exe и закрывает csgo_singleton_mutex.
    """
    pids: list[int] = []
    if primary_pid:
    ...
    for cs2_pid in pids:
        try:
            if _close_tf_singleton_mutex(cs2_pid):
                closed_any = True
...
class Account:
    ...
    def _kill_cs2_mutex(self, pid: int) -> None:
        try:
            # cs2ch.exe закрывал mutex не у одного PID, а у всех tf.exe.
